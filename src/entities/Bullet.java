...
public class Bullet extends Entity{
		...
		for(int i = 0; i < world.getZombiesAndBullets().size(); i++){
			...
			if(zombie.getBounds().contains(center)){
				...
				for(int j = 0; j <= 360; j+=15){
					...
					world.getParticles().add(new Particle(new Vector2D(position.getX(), position.getY()),size, life, speed, j, Color.RED));
				}
				zombie.hit();
		...
		for(int i = 0; i < world.getObstacles().size(); i++){
			...
			if(o.getBounds().contains(center)){
				for(int j = 0; j <= 360; j+=15){
					...
					world.getParticles().add(new Particle(new Vector2D(position.getX(), position.getY()),size, life, speed, j, new Color(95, 45, 0)));
				}
				world.getZombiesAndBullets().remove(this);
			}	
		}
	...
	public void render(Graphics g) {		
		...
		g.fillOval((int)(position.getX() - BULLETDIAMETER/2),(int)(position.getY() - BULLETDIAMETER/2),BULLETDIAMETER, BULLETDIAMETER);
	}
}
