...
public class Player extends Creature{
	...
	public Player(Vector2D position, double maxVelocity, World world) {
		...
		pistol = new Gun(Assets.pistolSkin, pistolIdle, pistolReload, pistolShoot,pistolShootSound, pistolReloadSound, this, 700, 9, 2700);
		rifle = new Gun(Assets.ak47, rifleIdle, rifleReload, rifleShoot, rifleShootSound, rifleReloadSound, this, 100, 30, 9000);
	...
	public void update() {
		...
		if(KeyManager.two)
			currentGun = rifle;
		if(KeyManager.reload && currentGun.getRound() != currentGun.getBulletsPerRound() && currentGun.getTotalBullets() > 0)
			currentGun.reload();
		...
		creatureCenter = new Vector2D(shootPoint.getX() - (position.getX() + PLAYERSIZE/2), shootPoint.getY() - (position.getY() + PLAYERSIZE/2));
		
		newPoint = new Vector2D(creatureCenter.getX()*Math.cos(angle) - creatureCenter.getY()*Math.sin(angle), creatureCenter.getX()*Math.sin(angle) + creatureCenter.getY()*Math.cos(angle));
