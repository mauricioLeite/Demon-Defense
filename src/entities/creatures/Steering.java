...
public class Steering {
	...
	public Vector2D obstacleAvoidance(){
		...
		ahead = new Vector2D(position.getX() + width + velocity.getX()*MAXSEEAHEAD,position.getY() + height + velocity.getY()*MAXSEEAHEAD);
		ahead2 = new Vector2D(position.getX() + width + velocity.getX()*MAXSEEAHEAD*0.5,position.getY() + height + velocity.getY()*MAXSEEAHEAD*0.5);
		zombiePos = new Vector2D(position.getX() + width,position.getY() + height);
	...
	private boolean collision(Vector2D ahead, Vector2D ahead2, Vector2D zombiePos, Tree obstacle){
		...
		return distance(center, ahead) <= obstacle.getRadius() + 15 || distance(center, ahead2) <= obstacle.getRadius() + 15 || distance(center, zombiePos) <= obstacle.getRadius() + 15;
	}
	...
	private Tree findObstacle(){
		...
		for(int i = 0; i < player.getWorld().getObstacles().size(); i++){
			...
			if(collision && (obstacle == null || distance(obstacle.getCenter(), zombie.getPosition()) > distance(o.getCenter(), zombie.getPosition()))){
				obstacle = o;
			}
		}
		return obstacle;
	...
	public void render(Graphics g){
		...
		Vector2D velocity = zombie.getVelocity();
		velocity = velocity.normalize();
		g.drawLine((int)(position.getX() - x+ zombie.getWidth()/2),  (int)(position.getY() - y + zombie.getHeight()/2), (int)(ahead.getX() - x),(int)(ahead.getY() - y));
		g2d.setColor(Color.magenta);
		g2d.drawLine((int)(position.getX() - x + zombie.getWidth()/2),  (int)(position.getY() - y + zombie.getHeight()/2), (int)(position.getX()  - x + zombie.getWidth()/2 + avoidance.getX()*20), (int)(position.getY() - y + zombie.getHeight()/2 + avoidance.getY()*20));
	}
}